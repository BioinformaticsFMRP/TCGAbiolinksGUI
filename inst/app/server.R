...
TCGAbiolinksGUIServer <- function(input, output, session) {
    ...
    output$downloadData <- downloadHandler(
        filename = function() {
            file  <- basename(as.character(parseFilePaths(get.volumes(isolate({input$workingDir})), input$seeditfile)$datapath))
    ...
    observeEvent(input$seeditSEbt, {
        ...
        createAlert(session, "seeditAlert", "seeditAlertMessage", title = "File created", style =  "success",
                    content = paste0("Saved as: ",filename), append = FALSE)
    })
    observeEvent(input$seeditfile, {

        updateTextInput(session,"seeditfilename", value = paste0("edited_",
                                                                 basename(as.character(parseFilePaths(get.volumes(isolate({input$workingDir})), input$seeditfile)$datapath))))
    ...
    observe({
        ...
        shinyFileChoose(input, 'seeditfile', roots=get.volumes(input$workingDir), session=session,
