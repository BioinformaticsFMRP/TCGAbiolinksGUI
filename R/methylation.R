...
diffmean <- function(group1, group2) {
    ...
    g1.g2 <- merge(g1[, c("probeID", "mean.g1")], g2[, c("probeID",
        "mean.g2")], by = "probeID")
...
survivalPlot <- function(met.md, legend = "Legend", cutoff = 0,
    ...
    xlab = "TIME SINCE DIAGNOSIS", time.reference = "DAYS", filename = "survival.pdf",
    color = c("green", "firebrick4", "orange3", "blue"), default.plot = "ggplot") {
    ...
    if (default.plot == "plot") {
        plot(fit, lwd = 4, col = color, main = main, xlab = paste0(xlab,
            "(", time.reference, ")"), ylab = ylab, yscale = 100,
            bg = "black")
        box(col = "black", lwd = 3)
        par(xpd = TRUE)
        ...
        ## Using ggplot
        surv <- ggsurv(fit, CI = "def", plot.cens = FALSE, surv.col = "gg.def",
            cens.col = "red", lty.est = 1, lty.ci = 2, cens.shape = 3,
            back.white = TRUE, xlab = paste0(xlab, "(", time.reference,
                ")"), ylab = ylab, main = main)
        if (cutoff != 0) {
            surv <- surv + ggplot2::coord_cartesian(xlim = c(0,
                cutoff))
        }
        ...
        surv <- surv + geom_point(aes(colour = group), shape = 3,
            size = 2)
        surv <- surv + ggplot2::guides(linetype = FALSE) + scale_y_continuous(labels = percent)
...
#'   met.mean.boxplot(data)
#'}
met.mean.boxplot <- function(data, sort = FALSE, filename = "G-CIMP-mean.methylation.pdf",
    ylab = "Mean DNA methylation", xlab = "DNA Methylation Clusters",
    title = "Mean DNA methylation by cluster", legend = "Legend",
    color = c("green", "red", "purple", "orange", "salmon", "grey")) {

    # Plot for methylation analysis Axis x: LGm clusters Axis y:
    # mean methylation
    if (sort) {
        p <- ggplot(data, aes(reorder(factor(cluster), avg),
            avg))
        p <- p + geom_boxplot(aes(fill = reorder(factor(cluster),
            avg)), notchwidth = 0.25) + geom_jitter(height = 0,
            position = position_jitter(width = 0.1), size = 3) +
            scale_fill_manual(values = color, labels = levels(reorder(factor(box.plot$cluster),
                box.plot$avg)), name = legend)
    } else {
        p <- ggplot(data, aes(factor(cluster), avg))
        p <- p + geom_boxplot(aes(fill = factor(cluster)), notchwidth = 0.25) +
            geom_jitter(height = 0, position = position_jitter(width = 0.1),
                size = 3) + scale_fill_manual(values = color,
            labels = levels(factor(data$cluster)), name = legend)
    }
    p <- p + ylab(ylab) + xlab(xlab) + labs(title = title) +
        theme(axis.title.x = element_text(face = "bold", size = 20),
            axis.text.x = element_text(angle = 90, vjust = 0.5,
                size = 16), axis.title.y = element_text(face = "bold",
                size = 20), axis.text.y = element_text(size = 16),
...
#' @import ggplot2
#' @export
volcano.plot <- function(data, filename = "volcano.pdf", ylab = "- 1*log10 of the Significance",
    xlab = "DNA Methylation", title = "Volcano Plot TCGA GBM Tumors",
    legend = "Legend", color = c(`1` = "black", `2` = "green",
        `3` = "red"), label = c(`1` = "Not Significant", `2` = "Hypermethylated",
        `3` = "Hypomethylated"), xlim = NULL, ylim = NULL, p.cut = 0.05,
    diffmean.cut = 0) {
    .e <- environment()
    data$threshold <- "1"
    ...
    # Plot a volcano plot
    p <- ggplot(data = data, aes(x = diffmean, y = -1 * log10(p.value.adj),
        colour = threshold), environment = .e) + geom_point()
    if (!is.null(xlim)) {
        p <- p + xlim(xlim)
    }
    if (!is.null(ylim)) {
    ...
    p <- p + geom_vline(aes(xintercept = -diffmean.cut), colour = "black",
        linetype = "dashed") + geom_vline(aes(xintercept = diffmean.cut),
        colour = "black", linetype = "dashed") + geom_hline(aes(yintercept = -1 *
        log10(p.cut)), colour = "black", linetype = "dashed")
    p <- p + scale_color_manual(breaks = c("1", "2", "3"), values = color,
        labels = label, name = legend)
    # saving box plot to analyse it
...
get.GRCh.bioMart <- function() {
    ...
    gene.location <- getBM(attributes = c("ensembl_gene_id",
        "ensembl_transcript_id", "chromosome_name", "start_position",
        "end_position", "strand", "external_gene_name", "external_transcript_name",
        "external_gene_source", "external_transcript_source_name",
        "hgnc_id", "entrezgene"), filters = c("chromosome_name"),
...
starbursanalysis <- function(met, expression) {
    ...
    gene.GR <- GRanges(seqnames = paste0("chr", gene.location$chromosome_name),
        ranges = IRanges(start = gene.location$start_position,
            end = gene.location$end_position), strand = gene.location$strand,
        symbol = gene.location$external_gene_name, EntrezID = gene.location$entrezgene)

    probe.info <- GRanges(seqnames = paste0("chr", met$Chromosome),
        ranges = IRanges(start = met$Genomic_Coordinate, end = met$Genomic_Coordinate),
    ...
    data.nearest.gene <- cbind(met, gene.order.by.distance[,
        c("external_gene_name", "entrezgene", "distance")])

    # volcano$gene <- met$Gene_Symbol volcano$cgID <-
    # rownames(volcano)
    volcano <- merge(met, expression, by.x = "Gene_Symbol", by.y = "GeneSymbol",
        incomparables = NA)
    volcano$ID <- paste(volcano$Gene_Symbol, volcano$probeID,
        volcano$cgID, sep = ".")

    # Preparing gene expression
    volcano$geFDR <- log10(volcano$FDR)
    volcano$geFDR2 <- volcano$geFDR
    volcano[volcano$dm > 0, "geFDR2"] <- -1 * volcano[volcano$dm >
        0, "geFDR"]

    # Preparing methylation
    volcano$meFDR <- log10(volcano$p.value.adj)
    volcano$meFDR2 <- volcano$meFDR
    volcano[volcano$diffmean > 0, "meFDR2"] <- -1 * volcano[volcano$diffmean >
        0, "meFDR"]
...
starburstplot <- function(data, filename = "volcano.pdf", ylab = paste0("Gene Expression\nlog10 of the",
    ...
    c <- subset(volcano.m, geFDR2 > lowerThr & geFDR2 < upperThr &
        meFDR2 < lowerThr)

    # Group 5: hypermethylated
    d <- subset(volcano.m, geFDR2 > lowerThr & geFDR2 < upperThr &
        meFDR2 > upperThr)

    # Group 6: upregulated
    e <- subset(volcano.m, geFDR2 > upperThr & meFDR2 < upperThr &
        meFDR2 > lowerThr)
    ...
    ## starburst plot
    p <- ggplot(data = volcano.m, environment = .e, aes(x = meFDR2,
        y = geFDR2, colour = threshold.starburst, size = threshold.size)) +
        geom_point()
    if (!is.null(xlim)) {
        p <- p + xlim(xlim)
    }
    ...
    p <- p + scale_color_manual(values = color, labels = label,
        name = legend)
    p + geom_hline(aes(yintercept = lowerThr)) + geom_hline(aes(yintercept = upperThr)) +
        geom_vline(aes(xintercept = lowerThr)) + geom_vline(aes(xintercept = upperThr))
...
heatmap.plus.sm <- function(x, Rowv = NULL, Colv = if (symm) "Rowv" else NULL,
    ...
    if (doCdend) {
        if (inherits(Colv, "dendrogram"))
            ddc <- Colv else if (identical(Colv, "Rowv")) {
            if (nr != nc)
    ...
    labRow <- if (is.null(labRow))
        if (is.null(rownames(x)))
            (1:nr)[rowInd] else rownames(x) else labRow[rowInd]
    labCol <- if (is.null(labCol))
        if (is.null(colnames(x)))
            (1:nc)[colInd] else colnames(x) else labCol[colInd]
    if (scale == "row") {
    ...
    if (doRdend)
        plot(ddr, horiz = TRUE, axes = FALSE, yaxs = "i", leaflab = "none") else frame()
    par(mar = c(0, 0, if (!is.null(main)) 1 else 0, margins[2]))
    if (doCdend)
        plot(ddc, axes = FALSE, xaxs = "i", leaflab = "none") else if (!is.null(main))
        frame()
    if (!is.null(main))
    ...
    if (key) {
        ...
        if (scale == "row")
            ...
            mtext(side = 1, "Column Z-Score", line = 2) else mtext(side = 1, "Value", line = 2)
