...
load.encode <- function(){
  ...
  for (j in 1:dim(url)[1]){
    ...
    for (i in 1:length(data)){
      ...
      encodeData <- rbind(encodeData, c(accession, biosample, assay,
                                        lab, target, description, url[j,2]))
...
load.roadmap <- function(){
  dataRoadmap <- read.csv(paste0("http://www.ncbi.nlm.nih.gov/geo/roadmap/",
                                 "epigenomics/?view=samples&sort=acc&mode=csv"),
...
load.biosamples <- function(env){
  ...
  downloader::download(paste0(gdocs,"28842439"), destfile = 'roadmaptab.tsv')
  downloader::download(paste0(gdocs,"514976736"), destfile = 'encodetab.tsv')
  downloader::download(paste0(gdocs,"0"), destfile = 'tcgatab.tsv')
  ...
  env$biosample.roadmap <- data.frame(lapply(biosample.roadmap,
                                             as.character),
                                      stringsAsFactors = FALSE)
  env$biosample.tcga <- data.frame(lapply(biosample.tcga,
                                          as.character),
                                   stringsAsFactors = FALSE)
...
load.systems <- function(env){
  ...
  downloader::download(url = paste0(gdocs, "1660203777"),
                       destfile = 'systemstab.tsv')
...
load.platforms <- function(env){
  gdocs <- paste0("https://docs.google.com/spreadsheets/d/10GwiiO8A4Ld1h4HTGO",
                  "88oaP7y3sqJHNRiQ_wcnKfXyM/export?format=tsv&id=10GwiiO8A4L",
                  "d1h4HTGO88oaP7y3sqJHNRiQ_wcnKfXyM&gid=")
  downloader::download(paste0(gdocs,"1664832653"),
                       destfile = 'platformstab.tsv')
