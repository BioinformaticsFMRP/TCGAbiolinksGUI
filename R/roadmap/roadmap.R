...
# X[[2]] = Term "Rattus norvegicus"
# X[[1]] = Field [Project]
my_ellipsis_function <- function(...) {
  input_list <- list(...)
  output_list <- lapply(X=input_list, function(x) {
      if(length(x) > 2){
        paste(paste(paste(x[[3]],gsub(" ","+",x[[2]]),sep="+"),x[[1]],sep="["),NULL,sep="]")
      }
      else{
        paste(paste(x[[2]],x[[1]],sep="["),NULL,sep="]")
      }
  })
  return(output_list)
}
...
geoDownloader <- function(iSearch, iType, iTarget,
  ...
  # Constant parameters
  roadmapPath  <- "http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=gds&term="
