...
install.packages(c("UpSetR","shiny","readr","googleVis","shinydashboard"))
...
After the search, the user can download the data by selecting the lines that will be downloaded, if no lines are 
select, all lines from the result will be downloaded.
...
files. The user can select from the table which MAF file to download. The output will be a R object (rda) file.

## Menu: TCGA Data information

In this menu the user can view a summary of the TCGA data.
...
* Level filter: Level of data selection.Only one can be selected.
* Sample type: Filter samples by types. Only used in "Platform vs samples" plot.
* Add counts label to barchart: Add numbers for each bar in the plot. Only used in "Type of samples" plot.
...
##### Data 
A cvs or R object (rda) file with the subtype information.
...
* Size title left barplot 
* Text size cluster names 
* Legend text size 
* Title legend size 
...
#### Video
[![IMAGE ALT TEXT](http://img.youtube.com/vi/0AhQI2vfTGQ/0.jpg)](http://www.youtube.com/watch?v=0AhQI2vfTGQ "Oncoprint example")
...
* Select mee: select an R object (rda) file with a mee object
* Select results: select an R object (rda) file with the results from ELMER analysis.
