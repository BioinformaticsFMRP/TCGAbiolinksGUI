...
TCGA data. This package offers a graphical user interface to the R/Biocondcutor package [TCGAbiolinks]( 	http://bioconductor.org/packages/TCGAbiolinks/) [@ref2] and [ELMER](http://bioconductor.org/packages/ELMER/) [@ref4].
...
If the Summarized Experiment object is selected, then the subytpe and clinical information will be added.
...
To highlight specific genes/probes consider using the "Highlightining option".
...
* Save file with results: Create a file with the same pattern as the one in the input, but with the new thersholds.
...
In this sub-menu the user will be able to perform a gene ontology enrichemnt analysis for the following processes:
biological, cellular component, molecular function. In additon, a network analysis for the groups of genes will be performed.
...
* Select annotation file: Select a R object (rda)  with the metadata information. Columns "patient" or "bcr_patient_barcode" should exist.
...
The options Mean DNA methylation difference threshold and Log FC threshold are used to circle genes which pass the cut-offs prevoiusly defined (eg. mean methylation or FDR).
...
* DNA methylation: Cut-off NA samples (%): By default, for the DNA methylation data will remove probes with NA values in more than 20% samples and remove the anottation data.
