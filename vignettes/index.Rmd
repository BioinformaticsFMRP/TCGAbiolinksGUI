...
and add `R_MAX_NUM_DLLS=120` in the end. Or you can run in R the following command:
`system(' echo "R_MAX_NUM_DLLS=150" >> /Library/Frameworks/R.framework/Resources/etc/Renviron')`

For UBUNTU please modify the file `/usr/local/lib/R/etc/Renviron` and add `R_MAX_NUM_DLLS=120` in the end.
Or you can run in R the following command:
`system(' echo "R_MAX_NUM_DLLS=150" >> /usr/local/lib/R/etc/Renviron')`
